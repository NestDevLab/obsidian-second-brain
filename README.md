...
- `openpack.json` declares portable instructions distributed by [Agentwheel](https://github.com/NestDevLab/agentwheel); the existing Claude hook scripts remain reference sources rather than being mislabeled as semantic hook JSON.
- OpenPack also ships the cross-runtime `obsidian-memory` skill and attaches the dependency-free `obsidian_amf` client as a skill asset; no canonical checkout path is required at runtime.
